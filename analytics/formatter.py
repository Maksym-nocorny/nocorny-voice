...
def render_overview(s: Optional[OverviewSection]) -> str:
    ...
    return (
        ...
        f"<i>{_now_utc()}</i>\n\n"
        f"<b>Requests</b>\n"
        f"  • Last hour: {_fmt_int(s.this_hour)}\n"
        f"  • Last 24h:  {_fmt_int(s.last_24h)}\n"
        f"  • Last 7d:   {_fmt_int(s.last_7d)}\n\n"
        f"<b>Users</b>\n"
        f"  • DAU {_fmt_int(s.dau)}  •  WAU {_fmt_int(s.wau)}  •  MAU {_fmt_int(s.mau)}\n"
        f"  • {_fmt_int(s.total_users)} total, {_fmt_int(s.new_users_today)} new today\n\n"
